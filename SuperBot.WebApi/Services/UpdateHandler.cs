    ...
    ITranslationsService translationsService, IBotStateReaderService botStateReaderService) : IUpdateHandler
{
    ...
    private async Task HandleMessage(Message msg)
    {
        ...
        logger.LogInformation("Message sent with ID: {MessageId}", sentMessage?.MessageId);
    }

    private async Task<Message> HandleCommandAsync(TelegramDataForProcessing telegramData)
    {
        var command = telegramData.CommandName;
        var chatState = await botStateReaderService.GetChatStateAsync(telegramData.ChatId);
        var dialogState = chatState?.DialogState ?? DialogState.MainMenu;

        return command switch
        {
            var cmd when cmd == translationsService.KeyboardKeys.BuySteamGames => await ChangeDialogState(telegramData, DialogState.BuyGame, translationsService.Translation.NavigateToGamePurchase),
            _ when dialogState == DialogState.BuyGame => await BuySteamGames(telegramData),

            var cmd when cmd == translationsService.KeyboardKeys.Account => await OpenMyAccount(telegramData),
            var cmd when cmd == translationsService.KeyboardKeys.ReferralProgram => await OpenReferralProgram(telegramData),
            var cmd when cmd == translationsService.KeyboardKeys.Start => await OpenStart(telegramData),

            var cmd when cmd == translationsService.KeyboardKeys.SteamTopUp => await OpenTopUpBalance(telegramData),
            ...
            var cmd when cmd == translationsService.KeyboardKeys.TopUpBalance => await OpenTopUpAccountBalance(telegramData),
            _ when dialogState == DialogState.TopUpAccountWithData => await TopUpAccountBalance(telegramData),

            var cmd when cmd == translationsService.KeyboardKeys.WithdrawFunds => await OpenWithdrawalOfFunds(telegramData),
            ...
            var cmd when cmd == translationsService.KeyboardKeys.Investments => await OpenInvestments(telegramData),
            var cmd when cmd == translationsService.KeyboardKeys.StartInvest => await StartInvestments(telegramData),
    ...
    private Task<Message> StartInvestments(TelegramDataForProcessing data) =>
        mediator.Send(new StartInvestmentCommand { ChatId = data.ChatId });

    private Task<Message> EnterInvestmentAmount(TelegramDataForProcessing data) =>
        mediator.Send(new EnterInvestmentAmountCommand { ChatId = data.ChatId, Amount = decimal.Parse(data.Text), UserId = data.UserID });
    ...
    private Task<Message> PrepareWithdrawalOfFunds(TelegramDataForProcessing data) =>
        mediator.Send(new PrepareWithdrawalOfFundsCommand { ChatId = data.ChatId, UserId = data.UserID, ChoseCard = data.Text });

    private Task<Message> WithdrawalOfFunds(TelegramDataForProcessing data) =>
        mediator.Send(new WithdrawalOfFundsCommand { ChatId = data.ChatId, UserID = data.UserID, DestinationType = "card", Amount = decimal.Parse(data.Text) });

    private Task<Message> OpenTopUpAccountBalance(TelegramDataForProcessing data) =>
        mediator.Send(new OpenTopUpAccountCommand { ChatId = data.ChatId });

    private Task<Message> TopUpAccountBalance(TelegramDataForProcessing data) =>
        mediator.Send(new TopUpAccountCommand { ChatId = data.ChatId, UserId = data.UserID, Amount = decimal.Parse(data.Text) });

    private Task<Message> OpenTopUpBalance(TelegramDataForProcessing data) =>
        mediator.Send(new OpenTopUpSteamCommand { ChatId = data.ChatId });

    private Task<Message> PrepareTopUpBalance(TelegramDataForProcessing data) =>
        mediator.Send(new PrepareTopUpSteamCommand { ChatId = data.ChatId, UserId = data.UserID, SteamLogin = data.Text });

    private Task<Message> TopUpBalance(TelegramDataForProcessing data) =>
        mediator.Send(new TopUpSteamCommand { ChatId = data.ChatId, UserId = data.UserID, Amount = decimal.Parse(data.Text) });

    private Task<Message> OpenStart(TelegramDataForProcessing data) =>
        mediator.Send(new OpenStartCommand { ChatId = data.ChatId, UserId = data.UserID, Username = data.FromUsername });

    private Task<Message> OpenMyAccount(TelegramDataForProcessing data) =>
        mediator.Send(new OpenMyAccountCommand { ChatId = data.ChatId, UserID = data.UserID, Name = data.UserFirstName });

    private Task<Message> OpenReferralProgram(TelegramDataForProcessing data) =>
        mediator.Send(new OpenReferralProgramCommand { ChatId = data.ChatId, UserId = data.UserID });

    private Task<Message> ChangeDialogState(TelegramDataForProcessing data, DialogState state, string text) =>
        mediator.Send(new ChangeDialogStateCommand { ChatId = data.ChatId, DialogState = state, Text = text });

    private async Task<Message> Usage(TelegramDataForProcessing data)
    {
        var chatState = await botStateReaderService.GetChatStateAsync(data.ChatId);
        return chatState?.DialogState switch
        {
            DialogState.MainMenu => await GetMainMenu(data),
            DialogState.BuyGame => await ChangeDialogState(data, DialogState.MainMenu, translationsService.Translation.BotMenu),
            _ => throw new NotImplementedException()
        };
    }

    private Task<Message> GetMainMenu(TelegramDataForProcessing data) =>
        mediator.Send(new GetMainMenuCommand { ChatId = data.ChatId });

    private Task<Message> BuySteamGames(TelegramDataForProcessing data) =>
        mediator.Send(new BuyGameCommand { ChatId = data.ChatId, FromUsername = data.FromUsername, Text = data.Text });
